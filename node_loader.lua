...
local function loadNodesFromChoices(parentNode)
    for _, choice in pairs(parentNode.choices) do
        ...
        -- removendo recursidade para adicionar o simulador de batalha
...
function nodeLoader.loadNodes()
    ...
    for id, node in pairs(nodeDictionary) do
        for _, choice in pairs(node.choices) do
            local destinationId = choice.destination
            if destinationId == nil or destinationId == "" then
                -- Sem destino: permitido apenas se a escolha possuir uma rotina
                if not (choice.routine ~= nil and type(choice.routine) == "function") then
                    warn(string.format("Uma das escolhas do node %s não possui destino nem rotina válida.", node.id))
                    hasErrors = true
                end
            else
